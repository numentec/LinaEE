    ...
    <MaterialCard class="mt-10">
      <template v-slot:heading>
        <v-toolbar dense color="secondary" class="mx-1" dark flat>
          ...
          <v-toolbar-title>Inventario por Localización</v-toolbar-title>
          <v-spacer />
...
export default {
    ...
    try {
      const [resp0, resp1] = await Promise.all([
        ...
        $axios.get('accviewconf-list/', {
          params: { idvista: '21', groups: groupList },
        }),
      ])
  ...
  computed: {
    ...mapState('linabi/storagexloc', ['curStore']),
    ...mapGetters('linabi/storagexloc', ['getCurCatalog', 'getCurStore']),
    curGrid() {
  ...
  methods: {
    ...mapActions('linabi/storagexloc', ['fetchData', 'addToCurCatalog']),
    ...
    exportGrid(opc) {
      ...
      if (selectedRows.length > 0) {
        // Exportar a Excel
        if (opc === 1) {
          const savingFilename = 'Storagexloc.xlsx'
